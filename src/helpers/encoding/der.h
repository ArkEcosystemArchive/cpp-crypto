...
#define DEFAULT_PRIVATEKEY_SIZE 32u

/**
 * DER Encode/Decode Helpers
 **/
static inline std::vector<uint8_t>& toDER(std::vector<uint8_t>& buffer) {
...
static inline void encodeDER(const std::vector<uint8_t>& r, const std::vector<uint8_t>& s, std::vector<uint8_t>& signature) {
    /* Adapted from https://github.com/bitcoinjs/bip66/blob/master/index.js */
    auto lenR = r.size();
    auto lenS = s.size();
...
static inline void encodeDER(uint8_t packed_signature[DEFAULT_PRIVATEKEY_SIZE * 2], std::vector<uint8_t>& signature) {
