...
void decodeDER(std::vector<uint8_t>& signature, std::vector<uint8_t>& r,
  ...
  assert(signature[1] ==
         signature.size() - 2);  // DER sequence length is invalid
  ...
  assert((lenR > 1));  // && (signature[4] == 0x00) && !(signature[5] == 0x80));
                       // // R value excessively padded

  assert(signature[lenR + 6] != 0x80);  // S value is negative
  assert(lenS > 1);  // && (signature[lenR + 6] != 0x00) && !(signature[lenR +
                     // 7] == 0x80)); // S value excessively padded
...
void encodeDER(const std::vector<uint8_t>& r, const std::vector<uint8_t>& s,
  ...
  assert((s[0] & 0x80) == 0);
  assert(lenR == 1 || r[0] != 0x00 ||
         (r[1] & 0x80) != 0);  // must have zero pad for negative number
