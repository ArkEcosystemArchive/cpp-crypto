...
Ark::Crypto::Identities::PublicKey::PublicKey(
    const char *const newPublicKeyStr) {
  memmove(this->bytes_, &HexToBytes(newPublicKeyStr).data()[0],
...
Ark::Crypto::Identities::PublicKey::fromPassphrase(
 ...
 **/
Ark::Crypto::Identities::PublicKey
Ark::Crypto::Identities::PublicKey::fromPrivateKey(PrivateKey privateKey) {
  std::vector<uint8_t> publicKey(COMPRESSED_PUBLICKEY_SIZE);
  const struct uECC_Curve_t *curve = uECC_secp256k1();
  uint8_t pub[64] = {};
  // TODO: using the current uECC implementation, a private key value of "1"
  // will return a false negative. It appears to be not supported given the
  // following issue: https://github.com/kmackay/micro-ecc/issues/128
  const uint8_t *privkeyBytes = privateKey.toBytes();
  auto ret = uECC_compute_public_key(
      &privkeyBytes[0], pub,
      curve);  // Don't check the return inline with the assert.  MSVC optimizer
               // does bad things.
  assert(ret != 0);
  assert(publicKey.size() == COMPRESSED_PUBLICKEY_SIZE);
  uECC_compress(pub, &publicKey[0], curve);
  return {BytesToHex(&publicKey[0], &publicKey[0] + COMPRESSED_PUBLICKEY_SIZE)
              .c_str()};
}
/**/
...
bool Ark::Crypto::Identities::PublicKey::validate(PublicKey publicKey) {
  auto publicKeyBytes =
      publicKey.toBytes();  // compressed publicKey bytes (uint8_t*)
  ...
  uECC_decompress(publicKeyBytes, uncompressedPublicKey,
                  curve);  // decompress the key
  return uECC_valid_public_key(uncompressedPublicKey,
                               curve);  // validate the uncompressed publicKey
}
/**/
