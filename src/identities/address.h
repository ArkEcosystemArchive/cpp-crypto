...
public:
  Address() : bytes_() {};
  Address(const char* newAddressStr);
  Address(const uint8_t* newAddressBytes);

  const uint8_t *toBytes();
  std::string toString() const;

  static Address fromPassphrase(const char* passphrase, uint8_t networkVersion);
  ...
  static bool validate(const char* addressStr, uint8_t networkVersion);
  ...
  static std::vector<uint8_t> bytesFromBase58Check(const char* address);
};
/**/
};  // namespace Identities
};  // namespace Crypto
