...
std::string Ark::Crypto::Transactions::Transaction::sign(
  ...
  this->senderPublicKey =
      Identities::PublicKey::fromPrivateKey(privateKey).toString();
...
bool Ark::Crypto::Transactions::Transaction::verify() const {
  return this->internalVerify(this->senderPublicKey, this->toBytes(),
                              this->signature);
}

bool Ark::Crypto::Transactions::Transaction::secondVerify(
    const char* secondPublicKey) const {
  return this->internalVerify(secondPublicKey, this->toBytes(false),
                              this->secondSignature);
}

std::vector<uint8_t> Ark::Crypto::Transactions::Transaction::toBytes(
    bool skipSignature, bool skipSecondSignature) const {
  ...
  bytes.insert(std::end(bytes), std::begin(senderKeyBytes),
  ...
  if (!this->recipientId.empty() && !skipRecipientId) {
    std::vector<std::uint8_t> recipientIdBytes =
        Address::bytesFromBase58Check(this->recipientId.c_str());
    bytes.insert(std::end(bytes), std::begin(recipientIdBytes),
  ...
  if (!this->vendorField.empty()) {
    bytes.insert(std::end(bytes), std::begin(this->vendorField),
  ...
  if (type == Enums::Types::SECOND_SIGNATURE_REGISTRATION) {
    const auto publicKeyBytes =
        HexToBytes(this->asset.signature.publicKey.c_str());
    bytes.insert(std::end(bytes), std::begin(publicKeyBytes),
                 std::end(publicKeyBytes));

  } else if (type == Enums::Types::DELEGATE_REGISTRATION) {
    bytes.insert(std::end(bytes), std::begin(this->asset.delegate.username),
  ...
  if (!skipSignature && !this->signature.empty()) {
    ...
    bytes.insert(std::end(bytes), std::begin(signatureBytes),
  ...
  if (!skipSecondSignature && !this->secondSignature.empty()) {
    ...
    bytes.insert(std::end(bytes), std::begin(secondSignatureBytes),
...
bool Ark::Crypto::Transactions::Transaction::internalVerify(
    std::string publicKey, std::vector<uint8_t> bytes,
