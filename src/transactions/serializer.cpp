...
// TODO: remove class ?
Serializer::Serializer(const Transaction& transaction)
...
std::string Serializer::serialize() {
  ...
  bytes.push_back(_transaction.network > 0
  ...
  std::vector<uint8_t> senderPublicKeyBytes =
      HexToBytes(_transaction.senderPublicKey.c_str());
  bytes.insert(bytes.end(), senderPublicKeyBytes.begin(),
...
void Serializer::serializeVendorField(std::vector<uint8_t>& bytes) {
  if (_transaction.vendorField.length() > 0) {
    auto vendorFieldLength =
        static_cast<uint8_t>(_transaction.vendorField.length());
    bytes.push_back(vendorFieldLength);
    bytes.insert(bytes.end(), std::begin(_transaction.vendorField),
                 std::end(_transaction.vendorField));

  } else if (_transaction.vendorFieldHex.length() > 0) {
    auto vendorFieldHexLength =
        static_cast<uint8_t>(_transaction.vendorFieldHex.length() / 2);
    bytes.push_back(vendorFieldHexLength);
    bytes.insert(bytes.end(), std::begin(_transaction.vendorFieldHex),
...
void Serializer::serializeTransfer(std::vector<uint8_t>& bytes) {
  ...
  std::vector<uint8_t> recipientIdBytes =
      Identities::Address::bytesFromBase58Check(
...
void Serializer::serializeSecondSignatureRegistration(
  ...
  std::vector<uint8_t> publicKeyBytes =
      HexToBytes(_transaction.asset.signature.publicKey.c_str());
...
void Serializer::serializeMultiSignatureRegistration(
  ...
  bytes.push_back(
      static_cast<uint8_t>(_transaction.asset.multiSignature.keysgroup.size()));
...
void Serializer::serializeSignatures(std::vector<uint8_t>& bytes) {
  if (_transaction.signature.length() > 0) {
    std::vector<uint8_t> signatureBytes =
        HexToBytes(_transaction.signature.c_str());
    bytes.insert(bytes.end(), signatureBytes.begin(), signatureBytes.end());
  }

  if (_transaction.secondSignature.length() > 0) {
    std::vector<uint8_t> secondSignatureBytes =
        HexToBytes(_transaction.secondSignature.c_str());
    bytes.insert(bytes.end(), secondSignatureBytes.begin(),
                 secondSignatureBytes.end());
  } else if (_transaction.signSignature.length() > 0) {
    std::vector<uint8_t> signSignatureBytes =
        HexToBytes(_transaction.signSignature.c_str());
    bytes.insert(bytes.end(), signSignatureBytes.begin(),
  ...
  if (!_transaction.signatures.empty()) {
    ...
    for (const auto& signature : _transaction.signatures) {
      ...
      bytes.insert(bytes.end(), std::begin(signatureBytes),
