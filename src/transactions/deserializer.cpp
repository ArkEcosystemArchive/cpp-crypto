...
void Deserializer::deserializeHeader(Transaction& transaction) {
  ...
  transaction.senderPublicKey =
      BytesToHex(this->_binary.begin() + 8, this->_binary.begin() + 33 + 8);
  ...
  if (vendorFieldLength > 0) {
    transaction.vendorFieldHex =
        this->_serialized.substr(vendorFieldOffset, vendorFieldLength * 2);
  }
...
void Deserializer::deserializeSecondSignatureRegistration(
  ...
  transaction.asset.signature.publicKey =
      this->_serialized.substr(_assetOffset, 66);
  _assetOffset += 66;
}
...
void Deserializer::deserializeDelegateRegistration(Transaction& transaction) {
  ...
  std::string username =
      this->_serialized.substr((_assetOffset / 2 + 1) * 2, usernameLength * 2);
...
void Deserializer::deserializeVote(Transaction& transaction) {
  ...
  for (uint8_t i = 0; i < voteLength; i++) {
    std::string vote =
        this->_serialized.substr(_assetOffset + 2 + i * 2 * 32, 68);
...
void Deserializer::deserializeSignatures(Transaction& transaction) {
  ...
  if (!signature.empty()) {
    ...
    transaction.signature =
        this->_serialized.substr(_assetOffset, signatureLength * 2);
    ...
    if (!transaction.secondSignature.empty()) {
      if (transaction.secondSignature.substr(0, 2) == "ff") {
        ...
        size_t secondSignatureLength =
            parseSignatureLength(transaction.secondSignature.substr(2, 2));
        transaction.secondSignature =
            transaction.secondSignature.substr(0, secondSignatureLength * 2);
        multiSignatureOffset += secondSignatureLength * 2;
      }
    }

    std::string signatures =
        this->_serialized.substr(_assetOffset + multiSignatureOffset);
    if (!signatures.empty() && signatures.substr(0, 2) == "ff") {
      signatures = signatures.substr(2);

      while (!signatures.empty()) {
        size_t multiSignatureLength =
            parseSignatureLength(signatures.substr(2, 2));
        if (multiSignatureLength > 0) {
          transaction.signatures.push_back(
              signatures.substr(0, multiSignatureLength * 2));
        }
...
void Deserializer::handleVersionOne(Transaction& transaction) {
  ...
  if (transaction.type == Enums::Types::VOTE) {
    const auto publicKey =
        Identities::PublicKey::fromHex(transaction.senderPublicKey.c_str());
    const auto address =
        Identities::Address::fromPublicKey(publicKey, transaction.network);
