...
Transaction Builder::buildTransfer(std::string recipientId, uint64_t amount, std::string vendorField, std::string passphrase, std::string secondPassphrase)
{
    ...
    return sign(transaction, passphrase, secondPassphrase);
}
...
Transaction Builder::buildDelegateRegistration(std::string username, std::string passphrase, std::string secondPassphrase)
{
    Transaction transaction;
...
Transaction Builder::buildVote(std::vector<std::string> votes, std::string passphrase, std::string secondPassphrase)
{
    ...
    const auto recipient = Identities::Address::fromPassphrase(passphrase.c_str(), Configuration::Network().get().version());
    transaction.recipientId = recipient.toString();

    return sign(transaction, passphrase, secondPassphrase);
}

Transaction Builder::buildMultiSignatureRegistration(uint8_t min, uint8_t lifetime, std::vector<std::string> keysgroup, std::string passphrase, std::string secondPassphrase)
{
    Transaction transaction;
