...
Transaction Builder::buildTransfer(std::string recipientId, uint64_t amount,
...
Transaction Builder::buildSecondSignatureRegistration(
  ...
  transaction.fee =
      Configuration::Fee().get(Enums::Types::SECOND_SIGNATURE_REGISTRATION);
...
Transaction Builder::buildDelegateRegistration(std::string username,
                                               std::string passphrase,
                                               std::string secondPassphrase) {
  Transaction transaction;
  transaction.type = Enums::Types::DELEGATE_REGISTRATION;
  transaction.fee =
      Configuration::Fee().get(Enums::Types::DELEGATE_REGISTRATION);
  transaction.asset.delegate.username = std::move(username);

  return sign(transaction, std::move(passphrase), std::move(secondPassphrase));
}

Transaction Builder::buildVote(std::vector<std::string> votes,
                               std::string passphrase,
                               std::string secondPassphrase) {
  Transaction transaction;
  ...
  const auto recipient = Identities::Address::fromPassphrase(
      passphrase.c_str(), Configuration::Network().get().version());
...
Transaction Builder::buildMultiSignatureRegistration(
    uint8_t min, uint8_t lifetime, std::vector<std::string> keysgroup,
    std::string passphrase, std::string secondPassphrase) {
  Transaction transaction;
...
Transaction Builder::sign(Transaction transaction, std::string passphrase,
                          std::string secondPassphrase) {
  transaction.timestamp =
      static_cast<uint32_t>(Utils::Slot::time(Configuration::Network().get()));
