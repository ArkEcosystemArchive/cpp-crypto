...
TEST(transactions, deserialize_second_signature_registration) {
  ...
  ASSERT_EQ(1, actual.version);
  ASSERT_EQ(30, actual.network);
  ASSERT_EQ(Ark::Crypto::Enums::Types::SECOND_SIGNATURE_REGISTRATION,
            actual.type);
  ...
  const auto publicKey = Ark::Crypto::Identities::PublicKey::fromHex(
      actual.senderPublicKey.c_str());
  const auto address = Ark::Crypto::Identities::Address::fromPublicKey(
      publicKey, actual.network);
...
TEST(transactions, deserialize_vote) {
  ...
  ASSERT_EQ(0, actual.expiration);
  ASSERT_STREQ(
      "16f28a180cd6f3ea46c10f358a457989e956e9d355258230d0c7b07acec10b73",
      actual.id.c_str());
  ASSERT_STREQ("D61mfSggzbvQgTUe6JhYKH2doHaqJ3Dyib",
               actual.recipientId.c_str());

  std::vector<std::string> votes = {std::string(
...
TEST(transactions, deserialize_multi_signature_registration) {
  ...
  ASSERT_EQ(Ark::Crypto::Enums::Types::MULTI_SIGNATURE_REGISTRATION,
            actual.type);
  ...
  for (uint8_t i = 0; i < keysgroup.size(); i++) {
    ASSERT_STREQ(keysgroup[i].c_str(),
                 actual.asset.multiSignature.keysgroup[i].c_str());
  }
