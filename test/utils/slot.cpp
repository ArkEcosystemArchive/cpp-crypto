...
TEST(utilities, slots_begin_epoch_time) {
  ...
  ASSERT_TRUE(std::chrono::duration_cast<std::chrono::seconds>(
                  std::chrono::milliseconds(
                      Ark::Crypto::Utils::Slot::begin_epoch_time(devnet)))
                  .count() == 1490101200ull);
#else
  ASSERT_EQ(1490101200ull,
...
TEST(utilities, slots_get_real_time) {
  ...
  ASSERT_TRUE(Ark::Crypto::Utils::Slot::real_time(10, devnet) ==
              1490101210000ull);
#else
